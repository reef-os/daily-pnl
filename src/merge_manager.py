...
def merge_pnl_coupa(df_pnl, df_coupa):
    print("Merging PNL and Coupa data...")
    new_rows = []
    df_mapping = pd.read_csv('mapping.csv').dropna(axis=1, how='all')
    ...
    last_df = fill_empty_celss(df_final)
    print("Merging PNL and Coupa data DONE!")
    return last_df


def merge_statement_merged_data(df_merged, df_statement):
