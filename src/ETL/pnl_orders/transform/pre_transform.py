class PreTransformer:
    ...
    def __group_data(melted_df):
        # burada gruplarken kaçıyor vessel_name boş olanlar
    ...
    def start_pre_transform(self, pn_orders_df):
        ...
        #df_filtered.to_csv("data/bronze/pnl_orders_filtered.csv")
        melted_df = self.__melt_data(df_filtered)
        #melted_df.to_csv("data/bronze/pnl_orders_melted.csv")
        grouped_df = self.__group_data(melted_df)
        #grouped_df.to_csv("data/bronze/pnl_orders_grouped.csv")
        return grouped_df
