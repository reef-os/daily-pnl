...
def clear_data(df):
    ...
    df = df[df['pl_mapping_1'].isin(['L1 Expenses', 'L2 Expenses', 'L3 Expenses', 'L4 Expenses', 'Non-recurring Costs'])]
...
def spread_corporate(main_df, corporate_df):
    ...
    filtered_array = [item for item in unique_vessel_names_list if'-' in item and not (item.split('-')[1] in ['900', '000'] or item.startswith('RHQ'))]
    ...
    date_unique_vessel_count = main_df.groupby('Business Date Local')['Vessel'].apply(lambda vessels: len(set(vessel for vessel in vessels if'-' in vessel and not (vessel.split('-')[1] in ['900', '000'] or vessel.startswith('RHQ'))))).to_dict()
